...
class _MyBotState extends State<bot> {
  ...
  void getDocId() async {
    ...
    userCollectionDocumenId =
        await ss.checkDataByEmail('adames.medi@gmail.com', 1);
  }
...
//location-02
  _getCurrentAddressOfUser() async {
    try {
      List<Placemark> place = await _geolocatorUser.placemarkFromCoordinates(
          _currentPositionOfUser.latitude, _currentPositionOfUser.longitude);
      Placemark placeOfUser = place[0];
      setState(() {
        _currentAddressOfUser =
            "${placeOfUser.locality}, ${placeOfUser.country}";
      });
    } catch (ex) {
      print(ex);
    }
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () async {
            FirebaseServiceSearch fs = FirebaseServiceSearch();
            String translated = await fs.engToSinhala('Hello world');
            print('translated - $translated');
            fs.updateUserFieldStatus(userCollectionDocumenId, 1, translated);
